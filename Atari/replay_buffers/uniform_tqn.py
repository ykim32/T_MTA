...
class UniformBuffer(object):
    ...
    def add(self, state, timeInv, action, reward, next_state, done, tdiscount):
        """Adds data to experience replay buffer."""
        if self._next_idx >= len(self.buffer):
            self.buffer.append((state, timeInv, action, reward, next_state,\
                                done, tdiscount))
        else:
            self.buffer[self._next_idx] = (state, timeInv, action, reward,\
                                           next_state, done, tdiscount)
    ...
    def sample(self, num_samples):
        ...
        states, timeInvs, actions, rewards, next_states, dones, tdiscounts =[],[],[],[],[],[], []
        idx = np.random.choice(len(self.buffer), num_samples)
        for i in idx:
            elem = self.buffer[i]
            state, timeInv, action, reward, next_state, done, tdiscount = elem
            states.append(np.array(state, copy=False))
            
            timeInvs.append(timeInv)
            
            actions.append(np.array(action, copy=False))
            rewards.append(reward)
            next_states.append(np.array(next_state, copy=False))
            dones.append(done)
            tdiscounts.append(tdiscount)
        states = torch.as_tensor(
            np.array(states).transpose(0, 3, 2, 1), device=self.device
        )
        timeInvs = torch.as_tensor(np.array(timeInvs, dtype=np.float32), device=self.device)
        ...
        tdiscounts = torch.as_tensor(np.array(tdiscounts, dtype=np.float32), device=self.device)
        return states, timeInvs, actions, rewards, next_states, dones, tdiscounts

    def sample_gym(self, num_samples):
        """Samples num_sample elements from the buffer."""
        states, timeInvs, actions, rewards, next_states, dones, tdiscounts =[],[],[],[],[],[], []
        idx = np.random.choice(len(self.buffer), num_samples)
        for i in idx:
            elem = self.buffer[i]
            state, timeInv, action, reward, next_state, done, tidscount = elem
            states.append(np.array(state, copy=False))
        ...
        dones = torch.as_tensor(np.array(dones, dtype=np.float32), device=self.device)
        tdiscounts = torch.as_tensor(np.array(tdiscounts, dtype=np.float32), device=self.device)
        return states, timeInvs, actions, rewards, next_states, dones, tidscounts


class DatasetBuffer(torch.utils.data.Dataset):
    def __init__(self, size, device):
        super(DatasetBuffer).__init__()
        # buffer: [state , timeInv, action, reward, next_state, done]
        self.buffer = [(np.random.randint(255,size=(84, 84, 4),dtype=np.uint8),\
           0, 0, 0, np.random.randint(255, size=(84, 84, 4), dtype=np.uint8), 0)]
    ...
    def __getitem__(self, idx):
        st, act, rew, n_st, d, tdis =  self.buffer[idx]
        ...
        tdiscount = torch.from_numpy(np.array(tdis, dtype=np.float32))
        return state, action, reward, next_state, done, tdiscount

    def add(self, state, timeInv, action, reward, next_state, done, tdiscount):
        if self._next_idx >= len(self.buffer):
            self.buffer.append((state, timeInv, action, reward, next_state,done, tdiscount))
        else:
            self.buffer[self._next_idx] = (state, timeInv, action, reward,\
                                           next_state, done, tdiscount)
        self._next_idx = (self._next_idx + 1) % self._size
...
class IterDatasetBuffer(torch.utils.data.IterableDataset):
    ...
    def add(self, state, timeInv, action, reward, next_state, done):
        """Adds data to experience replay buffer."""
        self.buffer.append((state, timeInv, action, reward, next_state, done))
    ...
    def sample_gen(self, start, end):
        while True:
            ...
            timeInvs = torch.from_numpy(np.array(timeInv, dtype=np.float32))
            ...
            tdiscounts = torch.from_numpy(np.array(tdiscount, dtype=np.float32))
            yield states, timeInvs, actions, rewards, next_states, dones, tdiscounts, i
...
#         ):
#             # elem = self.buffer[i]
#             state, timeIntv, action, reward, next_state, done = elem
#             states = torch.as_tensor(
#                 np.array(state, copy=False), device=self.device
#             ).transpose(0, 2)
#             timInvs = torch.as_tensor(np.array(timeInv, dtype=np.float32),\
#                                       device=self.device)
# 
#             actions = torch.as_tensor(np.array(action, copy=False), \
#                                       device=self.device)
#             rewards = torch.as_tensor(
...
#             )
#             yield states, timeInvs, actions, rewards, next_states, dones, index
